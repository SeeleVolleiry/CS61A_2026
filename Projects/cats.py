...
def about(keywords: list[str]):
    ...
    def is_contain(paragraph):
        ph = split(lower(remove_punctuation(paragraph)))
...
def accuracy(entered: str, source: str) -> float:
    ...
    minimum = min(len_entered, len_source)
    maximum = max(len_entered, len_source)
    # BEGIN PROBLEM 3
    if (not entered_words) and (not source_words):
        return 100.0
    elif (not entered_words) and source_words:
        return 0.0
    elif (not source_words) and entered_words:
        return 0.0
    ...
    return correct_counts / len_entered * 100.0
    # END PROBLEM 3
...
def wpm(entered: str, elapsed: int) -> float:
    ...
    # BEGIN PROBLEM 4
    words_avg_len = 5.0
    total_chars = len( entered ) # 计算字符数
    wpm = (total_chars / words_avg_len) / (elapsed / 60.0)
    return wpm
    # END PROBLEM 4
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    lowest_diff, smallest_index = limit + 1, 0
    for index in range(len(word_list)):
        diff_curr = diff_function(entered_word, word_list[index], limit)
        if diff_curr < lowest_diff:
            lowest_diff = diff_curr
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    ''' 循环版本，可惜题目不让用:(
    len_diff = abs(len(entered) - len(source))
    min_len = min(len(entered), len(source))
    mismatch = 0
    for i in range(min_len):
        if entered[i] != source[i]:
            mismatch += 1
            if mismatch + len_diff > limit:
                return limit + 1
