...
def calc_eval(exp):
    ...
    if isinstance(exp, Link):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(OPERATORS[operator], operands) # UPDATE THIS FOR Q2, what is type(operator)? Q3-Hint
...
def floor_div(args):
    ...
    20
    """
    if args.rest is Link.empty:
        return calc_eval(args.first)
    return floor_div(Link((calc_eval(args.first) // calc_eval(args.rest.first)), args.rest.rest)) # Q3-hint
...
def eval_and(expressions):
    ...
    if expressions is Link.empty:
        return True
    curr_exp = expressions
    last_val = scheme_t
    while curr_exp is not Link.empty:
        last_val = calc_eval(curr_exp.first)
        if last_val is scheme_f:
            return False
        curr_exp = curr_exp.rest
    return last_val
