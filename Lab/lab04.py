...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_to_a = distance(city, city_a)
    distance_to_b = distance(city, city_b)
...
def balanced(t):
    ...
    if is_leaf(t):
        ...
        sum_of_branches = [sum_tree(bs) for bs in branches(t)]
        for i in range(len(sum_of_branches) - 1):
            if sum_of_branches[i] != sum_of_branches[i+1]:
                return False
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    memory = [0] * (n + 1)
    memory[1] = 1
    for k in range(2, n + 1): # 从小到大计算，避免递归
        total = 0
        for i in range(1, k):
            total += memory[i] * memory[k - i]
        memory[k] = total
    return memory[n]
