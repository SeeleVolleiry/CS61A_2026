def inventory_pickup(inventory: list, items: list, capacity: int) -> list:
    ...
    ['world', 'hi', 'hello']
    """
    # 删除inventory中所有属于items的元素。
    # 将items中的元素一个个按顺序你添加到inventory末尾
    for item in list(items):
        while item in inventory:
          inventory.remove(item)
        inventory.append(item)
    # 判断长度是否超过capacity。超过就截断前面的部分，使之等于capacity。
    while len(inventory) > capacity:
        inventory.pop(0)
    return inventory
# 调试
# inv2 = [11, 12, 13]
# inv2_test = inventory_pickup(inv2, inv2, 7) # 两个参数都是inv2，修改inventory就会修改items，必须区分开。
# print(inv2_test)
...
def berry_finder(t):
    ...
    """
    find_target = 'berry'
    if label(t) == find_target:
        return True
    else:
        for branch in branches(t):
          if berry_finder(branch): # 不是用条件判断，直接return berry_finder(branch)，只会检查第一个子树。后面的无法被遍历。
              return True
        return False
...
def merge(incr_a, incr_b):
    ...
    is_out = "Iteratoe is exhausted"
    next_a, next_b = next(iter_a, is_out), next(iter_b, is_out)
    next_a, next_b = next_a, next_b
    last_of_merge = is_out
    while ((next_a is not is_out) or (next_b is not is_out)): # a,b都未耗尽时
        if (next_a is not is_out) and (next_b is is_out): # a没有耗尽，b耗尽
            # 比较 a接下来的值与已排列好的序列中最后一个值是否相等，相等则不加入继续next，更大则加入最后一个，更新最后一个值。
            if next_a != last_of_merge:
                yield next_a
            last_of_merge = next_a
            next_a = next(iter_a, is_out)
        elif (next_a is is_out) and (next_b is not is_out): # # b没有耗尽，a耗尽
            # 比较 b接下来的值与已排列好的序列中最后一个值是否相等，相等则不加入继续next，更大则加入最后一个，更新最后一个值。
            if next_b != last_of_merge:
                yield next_b
                last_of_merge = next_b
            next_b = next(iter_b, is_out)
        else: # 都未耗尽
            if next_a < next_b:
                yield next_a
                last_of_merge = next_a
...
def yield_paths(t, target):
    ...
    for b in branches(t):
        for path in yield_paths(b, target):
            yield [label(t)] + path # 不是label(t)，b的节点已经在path里了。
